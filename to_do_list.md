...
- Six-module selected-method Workbench shell in the React UI:
  ...
  - `regression.predict` and `doe.response_optimizer` expose top-level source catalogs and ID-only deep links while reusing the embedded prediction/optimizer components; both remain outside the generic analysis-run API. Legacy `regression.response_optimizer` records and URLs remain readable without rewrite
...
- `doe.response_optimizer` is an available dedicated DOE catalog method at version `0.3.0`; its top-level workspace follows RSM, selects checksum-validated stored RSM analyses, and keeps the existing embedded RSM entry. Config/result schema 2 and source-bundle schema 2 are unchanged, while legacy `regression.response_optimizer` records restore verbatim
