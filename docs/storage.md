...
- Analysis-run service code is split by storage-facing responsibility:
...
- `analysis_runs.py` remains the generic create/run dispatcher plus compatibility
  facade. Contract tests assert that stored-result loading, history, export, and
  comparison ownership stays in the sibling modules and that the facade does not
  regain direct storage metadata or result-execution persistence imports.
...
- `POST /api/v1/analysis-runs/{analysis_id}/exports/html` creates a checksum-recorded schema-2 `analysis_result_html_report` artifact under the analysis workspace after revalidating the stored result file. The report is a self-contained, print-friendly Statistical Twin analysis report with the user-facing summary, saved inputs, method-specific results, tables, and inline SVG charts before warnings and collapsed technical details. Graphical Summary and Equal Variances use dedicated stored-payload renderers; the current descriptive, normality, hypothesis, categorical, regression, and quality summaries use registered renderers with safe generic fallbacks. The raw saved result is available only in a closed `<details>` block as escaped JSON. Reports contain no scripts, external resources, raw data rereads, or internal relative/absolute filesystem paths. Existing schema-1 artifacts remain checksum-validated downloads and are never regenerated or rewritten.
