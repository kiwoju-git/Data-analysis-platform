...
- schema update API, 서버 페이지네이션 기반 row preview API, canonical JSONL rows/manifest materialization, duplicate-row와 memory estimate 및 persisted profile summary artifact를 포함한 profile/preflight API
...
범위:
1. richer date/time preflight 또는 route-level Workbench 중 하나만 선택
