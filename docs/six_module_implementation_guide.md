...
- 현재 frontend의 `App.tsx`는 API bootstrap, route state, analysis orchestration ownership을 유지하지만, 앱 sidebar/topbar/context layout은 `AppChrome`으로 분리되었고, dataset upload/paste/parsing/schema/preview/profile workflow state와 API handlers는 `useDatasetWorkflow`로 분리되어 있다. B0 파싱 확정, Dataset Context Bar, schema update, row preview, basic profile/preflight table 렌더링은 `DatasetPreparationPage`, `DatasetParsingPanel`, `DatasetVersionPanel`, profile/schema/preview section components, 그리고 `datasetDisplay` helper로 분리되어 있다. 분석 영역은 `WorkspaceRouter`, `AnalysisPage`, `AnalysisShell`, `AnalysisWorkbench`, `DescriptiveAnalysisPanel`, `GraphicalSummaryPanel`, `NormalityAnalysisPanel`로 분리되어 있고 URL 선택 상태는 `useAnalysisSelection`이 관리한다. `appRoute`가 루트/데이터셋 경로와 `/analysis/{module_id}/{method_id}` 경로를 구분하고 `WorkspaceRouter`가 데이터셋 page와 분석 page를 조건부 렌더링한다. 6개 모듈 planned/disabled shell, selected-method Workbench shell, 29개 method-specific guidance는 이 컴포넌트 경계 안에서 렌더링되며, 선택된 분석은 `/analysis/{module_id}/{method_id}` 경로와 legacy hash fallback으로 복원된다. 지원되는 AND filter 조건 UI는 Workbench 공통 위치로 이동했고, 현재 실행 요청 payload로 직렬화되는 메서드는 `eda.descriptive`, `eda.graphical_summary`, `eda.normality`다. 다음 UI slice에서는 새 실제 executable method가 추가될 때만 공통 filter state를 재사용하도록 연결하거나, 필요할 때만 route/layout 경계를 더 분리한다.
- 현재 backend 핵심 의존성에는 NumPy 2.2.6과 SciPy 1.15.3이 포함된다. `eda.descriptive`, `eda.graphical_summary`, basic XLSX worksheet parsing은 Python 표준 라이브러리 기반이고, `eda.normality`는 SciPy 기반 Shapiro-Wilk/Anderson-Darling 계산을 사용한다. pandas/statsmodels/openpyxl 등은 Windows/Python 3.10 호환성 스파이크 후 필요 시 lockfile을 갱신한다.
- 현재 분석 실행 API는 `eda.descriptive`, `eda.graphical_summary`, `eda.normality`를 실제 계산으로 실행하고, 다른 미구현 메서드는 거부한다. 통계 계산 mock result 또는 완료처럼 보이는 빈 결과 화면은 추가하지 않는다.
...
현재 구현 상태: `eda.graphical_summary`는 stdlib 기반 실제 계산으로 available 상태다. API는 canonical rows를 스트리밍해 histogram, boxplot, Q-Q, ECDF용 typed chart-data payload를 저장/반환하며, frontend는 최소 실행 패널과 결과 요약 테이블을 제공한다. 실제 차트 renderer, grouping/small multiple, KDE는 아직 구현하지 않는다.
...
현재 구현 상태: `eda.normality`는 available 상태다. NumPy 2.2.6/SciPy 1.15.3을 production dependency로 고정했고, API는 canonical rows를 스트리밍해 Shapiro-Wilk, Anderson-Darling, Q-Q point payload를 저장/반환한다. 정규성 결과만으로 후속 모수/비모수 검정을 자동 전환하지 않으며, grouped normality는 이번 slice에서 `normality_grouping_not_supported`로 거부한다. 상세 계약은 `docs/normality_method_contract.md`를 따른다.
...
현재 구현 상태: `eda.equal_variances`는 아직 `planned` 상태다. SciPy 기반 Levene/Brown-Forsythe 검정 의존성 검증과 기준 fixture가 완료되기 전까지 API는 실행을 거부해야 하며, p-value 또는 검정통계량을 mock으로 반환하면 안 된다.

Levene/Brown-Forsythe smoke calculation도 `scripts/check-stat-deps.ps1`의 일부다. 이 smoke가 통과하더라도 method available 전환에는 별도의 reference fixture, edge-case warnings, schema/result envelope 테스트가 필요하다.
