...
- schema update API, 서버 페이지네이션 기반 row preview API, canonical JSONL rows/manifest materialization, duplicate-row와 memory estimate를 포함한 profile/preflight API
...
현재 stdlib slice의 canonical 형식은 UTF-8 JSONL rows와 JSON manifest이다. SQLite에는 `dataset_artifacts`로 상대 경로, SHA-256, media type, byte size만 저장한다. 권장 고성능 canonical 형식은 여전히 Parquet이지만, `pyarrow`의 Python 3.10/Windows 호환성과 라이선스를 확인한 뒤 채택한다. 승인 전 임시 방편으로 pickle/joblib을 사용하지 않는다.
...
범위:
1. persisted profile artifacts, richer date/time preflight, 또는 route-level Workbench 중 하나만 선택
