...
현재 공통 UI에서는 여섯 모듈과 method 선택이 분석 화면의 첫 동작이다.
질문 기반 method 찾기와 역할 사전은 `/help`에 있고, 선택 method의 context
도움말은 분석 제목 옆 버튼에서 연다. `/reports`는 기존 generic analysis-run
JSON/CSV/HTML export를 중앙에서 찾고 생성·다운로드한다. Dedicated Predict,
Optimizer, RSM, Bayesian HTML을 지원한다고 표시하지 않으며 자세한 경계는
`docs/report_center_contract.md`를 따른다.
...
현재 구현 상태: `eda.normality`는 version `0.2.0`/result schema `2`의 available method다. NumPy 2.2.6/SciPy 1.15.3을 production dependency로 고정했고, API는 canonical rows를 스트리밍해 Shapiro-Wilk, SciPy Anderson-Darling 임계값 판단, Stephens unknown-mean/variance adjusted statistic과 근사 p-value, Q-Q point payload를 저장/반환한다. frontend는 AD p를 `근사`로 표시하고 Q-Q point를 pointer/touch/roving-keyboard로 탐색한다. 정규성 결과만으로 후속 모수/비모수 검정을 자동 전환하지 않으며, grouped normality는 `normality_grouping_not_supported`로 거부한다. 상세 계약은 `docs/normality_method_contract.md`를 따른다.
