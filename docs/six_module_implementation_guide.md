...
- 업로드 또는 붙여넣기 입력, header/headerless 파싱 확정, Dataset Context Bar, schema update, row preview, basic profile/preflight table, 6개 모듈 planned/disabled shell, shared `AnalysisWorkbench` component, selected-method Workbench shell과 method-specific guidance를 보여주는 최소 UI
...
1. cross-method filter 재사용과 공통 필터 UX
...
- 현재 frontend의 `App.tsx`는 B0 파싱 확정 UI, Dataset Context Bar, basic profile/preflight table, 그리고 첫 `eda.descriptive` 실행 패널을 한 화면에 유지한다. 6개 모듈 planned/disabled shell, selected-method Workbench shell, 29개 method-specific guidance는 shared `AnalysisWorkbench` component로 분리되어 있으며, 선택된 분석은 `/analysis/{module_id}/{method_id}` 경로와 legacy hash fallback으로 복원된다. `eda.descriptive`는 지원되는 AND filter 조건을 UI에서 만들 수 있다. 다음 UI slice에서는 Workbench를 더 깊은 route-level page/component와 추가 shared feature components로 분리하거나 filter control을 cross-method 계약으로 일반화한다.
- 현재 backend 핵심 의존성에는 외부 통계 라이브러리가 없다. `eda.descriptive`와 basic XLSX worksheet parsing은 Python 표준 라이브러리 기반으로 구현되어 있으며, NumPy/SciPy/statsmodels/openpyxl 등은 Windows/Python 3.10 호환성 스파이크 후 필요 시 lockfile을 갱신한다.
...
범위:
1. cross-method filter reuse 또는 deeper Workbench page/component split 중 하나만 선택
