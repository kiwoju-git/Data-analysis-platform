...
현재 구현 상태: `regression.linear_model`은 세 번째 Gate C1 available slice다. Canonical rows에서 숫자형 반응 컬럼 1개와 숫자형/범주형 main-effect 예측 컬럼 1개 이상을 complete-case로 읽고, intercept 포함 OLS 모형을 NumPy/SciPy로 계산한다. 범주형 예측변수는 안전한 term builder에서 첫 정렬 수준을 reference로 하는 treatment coding만 지원한다. 선택된 숫자형 predictor에 대해서는 명시적으로 요청한 2차항과 숫자형×숫자형 interaction을 추가할 수 있다. 결과는 사용 N과 제외 수, model specification, factor reference level, term source column IDs, 계수 estimate/SE/t/p/CI, R², adjusted R², residual standard error, F test, VIF, condition number, residual summary, leverage summary, Cook's distance summary, capped diagnostic points, non-causation/linearity/independence/homoscedasticity/residual-normality/outlier warnings, `analysis_row_snapshot` provenance, result SHA-256 persistence, safe JSON `regression_model_manifest` artifact, prediction basis metadata, `regression_models` metadata row, checksum-validated `GET /api/v1/regression-models/{model_id}` retrieval, `POST /api/v1/regression-models/{model_id}/prediction-preflight` schema/extrapolation/unseen-level preflight, backend `POST /api/v1/regression-models/{model_id}/predictions` predicted mean/CI/PI 계산, checksum-validated NDJSON 전체 행 저장과 paged retrieval, confirmed dataset version catalog/target 선택, 25행 prediction page 표시, inline residual/fitted and leverage/Cook diagnostic charts, 그리고 inline prediction interval chart를 포함한다. 범주형 interaction, factor×numeric interaction, no-intercept, HC3 robust covariance, exported residual/leverage chart artifacts는 아직 구현하지 않는다.
...
현재 구현 상태: stored app-created OLS model manifest에 대해 `POST /api/v1/regression-models/{model_id}/prediction-preflight`와 `POST /api/v1/regression-models/{model_id}/predictions`가 backend에 구현되어 있고, Linear Model UI에서 current version 또는 paged catalog에서 명시적으로 선택한 다른 confirmed dataset version을 대상으로 preflight 후 prediction을 실행할 수 있다. `GET /api/v1/dataset-versions` 목록은 정제된 파일명과 version ID/번호, 행·열 수, 생성 시각만 반환하며 raw row, storage path, SHA는 반환하지 않는다. Preflight endpoint는 manifest checksum, source row snapshot, target canonical rows, schema hash, 필수 predictor 컬럼 매핑, numeric training range, missing/non-numeric counts, and unseen categorical levels를 검증한다. Prediction endpoint는 같은 preflight path를 통과한 target dataset version에 대해서만 design matrix를 재구성하고 predicted mean, mean-response confidence interval, individual prediction interval, exclusion counts, warning metadata를 반환하며, `regression.predict` result envelope와 전체 prediction row NDJSON artifact를 SHA-256과 함께 저장한다. `GET /api/v1/regression-models/predictions/{prediction_id}/rows`는 최대 200행의 checksum-validated page를 반환한다. Backend API coverage는 cross-dataset execution, 1,005행 inline/page 경계, row artifact 변조, metadata 실패 cleanup, 저장 예측 결과 checksum mismatch, categorical treatment-coded term reconstruction, numeric quadratic/interaction reconstruction, and missing prediction-basis recovery를 포함한다. UI는 target 변경 시 이전 비동기 prediction state를 무효화하고 prediction summary와 25행 page table을 표시하되 raw target cell values는 응답, stored config, 화면에 포함하지 않는다.
...
현재 구현 상태: Gate D1 설계 자산과 response-entry slice가 구현되어 있다. `doe.factorial_design`은 catalog에서 available이지만 일반 분석 실행 API가 아니라 `POST /api/v1/doe-designs/factorial`, `GET /api/v1/doe-designs/{design_id}`, `PUT /api/v1/doe-designs/{design_id}/responses`, `GET /api/v1/doe-designs/{design_id}/responses`를 사용한다. 현재 지원 범위는 2-level full factorial 설계 생성, 2-6개 연속 factor의 low/high/unit, 반복, 센터점, optional block count, randomization on/off와 seed, standard/run order 보존, SQLite schema v7 design/version/run metadata 저장, schema v8 run response 저장/조회, `design_sha256` 검증 조회, 최소 Workbench 입력/preview/response-entry UI다. 효과 추정, OLS/ANOVA, residual diagnostics, alias structure, DOE 분석 차트는 아직 구현하지 않는다. 상세 계약은 `docs/factorial_design_method_contract.md`를 따른다.
...
- 현재 완료된 slice: 2-level full factorial design asset/version/run metadata, deterministic randomization seed, schema v7 storage, schema v8 response storage, dedicated design create/read/response API, 최소 Workbench preview/response-entry UI.
