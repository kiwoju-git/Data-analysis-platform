...
- 현재 backend 핵심 의존성에는 NumPy 2.2.6과 SciPy 1.15.3이 포함된다. `eda.descriptive`, `eda.graphical_summary`, `quality.individuals_chart`, `quality.subgroup_chart`, `quality.run_chart`, `quality.capability`, `quality.gage_run_chart`, basic XLSX worksheet parsing은 Python 표준 라이브러리 기반이고, `eda.normality`, `eda.equal_variances`, `hypothesis.one_sample_t`, `hypothesis.paired_t`, `hypothesis.one_sample_wilcoxon`, `hypothesis.two_sample_t`, `hypothesis.mann_whitney`, `hypothesis.kruskal_wallis`, `hypothesis.one_way_anova`, `hypothesis.equivalence_tost`, `categorical.one_proportion`, `categorical.two_proportion`, `categorical.chi_square_association`, `regression.pearson`, `regression.xy_correlation`, `regression.linear_model`, `quality.gage_rr`는 SciPy/NumPy 기반 계산을 사용한다. pandas/statsmodels/openpyxl 등은 Windows/Python 3.10 호환성 스파이크 후 필요 시 lockfile을 갱신한다.
...
현재 구현 상태: stored app-created OLS model manifest에 대해 dedicated prediction API가 구현되어 있고, Linear Model UI에서 current version 또는 paged catalog에서 선택한 다른 confirmed dataset version을 대상으로 실행한다. Preflight는 target mapping/range/category 검증 전에 source analysis 존재·method/version·stale 상태, fit/current source schema hash, source canonical artifact, model metadata와 manifest SHA를 검증하며 stale source model은 refit 전까지 차단한다. Prediction result provenance는 source analysis/source dataset/target dataset/model dependency와 runtime/build/package 정보를 기록하고 raw path, original filename, raw predictor values를 기록하지 않는다. Stored result restore, row page, 전체 CSV export/list/download는 analysis run config, result envelope, rows artifact/header, model record, manifest 사이의 ID/version/hash/count 관계를 공통 validator로 검증한다. UI의 history/export/comparison/restore/target/export/preflight/prediction/page hook은 실제 지연 응답 역전 테스트로 최신 상태와 loading reset을 검증한다. `regression.predict` generic method page는 disabled 상태를 유지하지만 registry 안내는 “저장된 회귀 모델을 이용한 예측은 회귀모형 적합 화면에서 지원되며 독립 Predict 화면만 아직 없다”는 현재 상태를 반영한다.
...
현재 구현 상태: generic analysis-run method는 disabled 상태를 유지하지만,
checksum이 검증된 `doe.response_surface` 결과를 사용하는 dedicated optimizer
API와 `ResponseSurfacePanel` 내부 UI가 구현되어 있다. Backend는 동일 factor
space를 공유하는 최대 8개 response objective를 지원하며, 현재 UI는 방금 적합한
RSM response 하나를 구성한다. method version `0.1.0`, config/result schema `1`,
schema-v9 DOE analysis storage를 사용한다. 상세 계약은
...
현재 구현 상태: `quality.attribute_control_chart`는 method version `0.1.0`의
available Phase I slice다. P/NP는 불량품 수와 양의 정수 표본 크기, C/U는
...
선택, C 동일 기회 확인, 관측별 한계 SVG와 결과표를 제공한다. 공통 stored
result/row snapshot 및 JSON/CSV/HTML export를 사용하며 chart image export와
Phase II frozen limits는 아직 지원하지 않는다. 상세 계약은
`docs/attribute_control_chart_method_contract.md`를 따른다.
...
현재 구현 상태: Gate D1 2-level full factorial 설계, response entry, 효과/OLS/ANOVA 분석 slice가 구현되어 있다. `doe.factorial_design`은 catalog에서 available이지만 일반 분석 실행 API가 아니라 dedicated design/response/analysis/report routes를 사용한다. 2-6개 연속 factor, 반복, center point, fixed block, deterministic randomization, standard/run order, schema v7-v9 metadata, design/response/result checksum, -1/+1 coding, hierarchy 고정 interaction, curvature, block effects, partial drop-one SS, pure error/lack-of-fit, residual/influence diagnostics, main/interaction plot payload, Workbench 효과/ANOVA UI와 HTML report를 지원한다. Fractional factorial alias analysis, optimizer, chart image export는 아직 구현하지 않는다. 상세 계약은 `docs/factorial_design_method_contract.md`를 따른다.
...
현재 구현 상태: Gate D2의 RSM 및 bounded optimizer slice가 구현되어 있다. 전용 API에서 2-5개 연속 factor의 rotatable central composite inscribed 또는 face-centered CCD를 생성하고, run별 response를 저장한 뒤 hierarchy가 고정된 full quadratic OLS를 적합한다. coefficient inference, partial drop-one ANOVA, pure error/lack-of-fit, residual/influence diagnostics, Hessian stationary-point 분류와 axial/factorial 영역 판정, 첫 두 factor의 21x21 contour payload, Workbench UI, schema-v9 checksum/dependency 저장을 지원한다. 적합 후 같은 화면에서 maximize/minimize/target/range desirability, 좁힌 factor bounds, 선형 부등식 제약, deterministic search budget을 사용하는 `regression.response_optimizer`를 실행하고 검증된 결과를 복원할 수 있다. 상세 계약은 `docs/response_surface_method_contract.md`와 `docs/response_optimizer_contract.md`를 따른다.
...
- 현재 완료된 slice: 2-level full factorial design/response/analysis metadata, deterministic randomization, schema v7-v9 storage, dedicated design/response/analysis/report API, 효과/OLS/ANOVA/pure error/lack-of-fit/diagnostics, Workbench 결과 UI.
...
현재 `doe.bayesian_optimization`은 `0.1.0` planning-only catalog entry다.
연속 factor 1~6개, 단일 maximize/minimize objective, 실제 관측 completed
trial history, Matérn-5/2 Gaussian Process, Expected Improvement, 실제 단위
선형 부등식 constraint, deterministic seed/trial/candidate/time budget을 첫
계약으로 고정한다. 앱이 목적함수, Python, shell 또는 장비를 실행하지 않고
추천을 관측값으로 자동 승격하지 않는다. 실행 API/result schema/storage는
아직 없으며 `docs/bayesian_optimization_contract.md`의 reference gate를
통과하는 별도 slice 전에는 available로 바꾸지 않는다.
