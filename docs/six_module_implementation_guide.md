...
현재 구현 상태: stored app-created OLS model manifest에 대해 `POST /api/v1/regression-models/{model_id}/prediction-preflight`와 `POST /api/v1/regression-models/{model_id}/predictions`가 backend에 구현되어 있고, Linear Model UI에서 active dataset version을 대상으로 preflight 후 prediction을 실행할 수 있다. Preflight endpoint는 manifest checksum, source row snapshot, target canonical rows, schema hash, 필수 predictor 컬럼 매핑, numeric training range, missing/non-numeric counts, and unseen categorical levels를 검증한다. Prediction endpoint는 같은 preflight path를 통과한 target dataset version에 대해서만 design matrix를 재구성하고 predicted mean, mean-response confidence interval, individual prediction interval, exclusion counts, warning metadata를 반환하며, `regression.predict` result envelope를 SHA-256과 함께 저장한다. Backend API coverage는 저장 예측 결과 checksum mismatch, categorical treatment-coded term reconstruction, numeric quadratic/interaction reconstruction, and missing prediction-basis recovery를 포함한다. UI는 prediction summary와 capped row table을 표시하되 raw target cell values는 응답, stored config, 화면에 포함하지 않는다.

다음 prediction slice는 `docs/regression_prediction_contract.md`의 남은 target dataset selection, paged result retrieval, independent reference fixture, and browser/E2E coverage를 만족해야 한다.
