...
현재 구현 상태: `quality.attribute_control_chart`는 method version `0.2.0`,
result schema `2`의 available Phase I/II slice다. P/NP는 불량품 수와 양의 정수 표본 크기, C/U는
...
3-sigma 한계를 적용한다. NP sample size와 C current opportunity 확인을 별도로
강제하며 target 행에서 한계를 재추정하지 않는다. 공통 stored result/row snapshot
및 JSON/CSV/HTML export는 limit-set/target 관계를 다시 검증한다. 기존
`0.1.0`/schema `1` 결과는 Phase I로 그대로 복원되며 재해석하지 않는다.
chart image export는 아직 지원하지 않는다. 상세 계약은
