...
`docs/attribute_control_chart_method_contract.md`를 따른다. Phase II monitoring은
아직 실행되지 않지만 `docs/attribute_control_chart_phase_2_contract.md`에
정의된 immutable limit-set asset schema `1`의 SQLite schema-13 저장소와
create/get/list API는 구현됐다. 첫 monitoring slice는 기존 Phase I `0.1.0`
결과를 재해석하지 않고 method `0.2.0`, result schema `2`로 분리한다.
...
현재 `doe.bayesian_optimization`은 dedicated API/UI에서 실행하는 `0.2.0`
