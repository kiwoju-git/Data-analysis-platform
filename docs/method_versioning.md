...
statistical method version or result schema. API contract version 6 prevents a
...
2/3/2, and `regression.response_optimizer` stays `0.3.0` with config/result/
source-bundle schemas 2/2/2. Existing stored artifacts retain their recorded
versions and are never rewritten during restore.
