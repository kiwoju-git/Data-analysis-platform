...
| Filter snapshot row freezing | Done for empty `eda.descriptive` filters | creates `analysis_row_snapshot` artifacts with filter hash, canonical artifact hash, and included row counts; non-empty conditions still reject |
...
1. Continue the Workbench split into dedicated route-level analysis pages or add a validated non-empty filter expression engine.
