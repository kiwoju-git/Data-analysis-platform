...
export const analysisMethodGuidance = {
  ...
  "regression.linear_model": {
    ...
    optionChecklist: [translationToken("reg.estimator"), "범주형 treatment coding", "숫자형 2차항/상호작용", "intercept 포함", translationToken("reg.guidance.olsOptions"), translationToken("reg.guidance.penaltyOptions"), "complete-case 결측 처리"],
    preflightChecks: [translationToken("reg.guidance.sampleBudget"), "상수 컬럼/단일 수준 factor", translationToken("reg.guidance.olsDesign")],
    resultFocus: ["계수 추정치", translationToken("reg.guidance.olsInference"), translationToken("reg.summary"), translationToken("reg.curve"), translationToken("reg.path")],
    plainLanguage: translationToken("reg.intro"),
    commonErrors: [
      "반응 변수와 예측변수에 같은 컬럼을 고른 경우",
      "예측변수 수에 비해 complete-case 행이 너무 적은 경우",
      "범주형 예측변수가 필터 후 한 수준만 남은 경우",
      translationToken("reg.guidance.olsDesign"),
      translationToken("reg.scaling"),
      translationToken("reg.assumptions"),
      translationToken("reg.pointOnly"),
