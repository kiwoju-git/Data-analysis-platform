import type { BayesianStudyCloseReason, BayesianStudyResponse } from "../../api";
...
export function BayesianStudyClosePanel({
  ...
  const blocker = bayesianStudyCloseBlocker(study, target, hasRecommendation);
  return (
    <section aria-labelledby="bayesian-study-close-title">
      ...
      {study.pending_trial_count > 0 ? <p className="cell-subtext">종료하려면 pending trial {study.pending_trial_count}개를 먼저 완료하거나 중단하세요.{target === "abandoned" ? " 중단 종료를 선택한 상태에서는 초기 trial도 종료 의도로 중단할 수 있습니다." : ""}</p> : target === "completed" && !hasRecommendation ? <p className="cell-subtext">완료 종료에는 최소 관측 수와 저장된 recommendation이 필요합니다.</p> : null}
