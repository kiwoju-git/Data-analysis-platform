...
export function InteractiveBoxplotChart({ boxplot, chartId, columnName }: InteractiveBoxplotChartProps) {
  ...
  const details = active === null ? [] : [
    { label: active.label, value: active.id === outlierEntry.id ? active.value.toLocaleString() : formatNumber(active.value) },
    { label: "IQR", value: formatNumber(q3 - q1) },
  ];

  return (
    <div className="interactive-chart">
      <svg aria-labelledby={`${chartId}-title ${chartId}-description`} className="chart-svg interactive-chart-svg" role="img" viewBox={`0 0 ${width} ${height}`}>
        <title id={`${chartId}-title`}>{`${columnName} 박스플롯`}</title>
        <desc id={`${chartId}-description`}>개별 outlier 값 없이 quartile, whisker, fence와 outlier count 집계만 표시합니다.</desc>
        <line className="chart-axis" x1={plot.left} x2={plot.left + plotWidth} y1={plot.top + plotHeight} y2={plot.top + plotHeight} />
        <line className="boxplot-fence" x1={x(boxplot.lower_fence as number)} x2={x(boxplot.lower_fence as number)} y1={y - 30} y2={y + 30} />
        <line className="boxplot-fence" x1={x(boxplot.upper_fence as number)} x2={x(boxplot.upper_fence as number)} y1={y - 30} y2={y + 30} />
        ...
        {[...entries, outlierEntry].map((entry) => {
          ...
          return <rect
            aria-label={`${columnName} ${entry.label}: ${entry.id === outlierEntry.id ? entry.value : formatNumber(entry.value)}`}
            ...
            width={entry.id === outlierEntry.id ? 72 : 18}
            x={itemX - (entry.id === outlierEntry.id ? 36 : 9)}
            y={itemY - (entry.id === outlierEntry.id ? 12 : 32)}
          ><title>{entry.label}</title></rect>;
        })}
        {boxplot.outlier_count > 0 ? <text className="chart-note" x={plot.left} y={plot.top + 20}>outliers {boxplot.outlier_count}</text> : null}
      ...
      {interaction.activeItem !== null && active !== null ? <ChartTooltip details={details} left={interaction.activeItem.left} title={active.label} top={interaction.activeItem.top} /> : null}
      <div className="chart-selected-detail" aria-live="polite">{active === null ? <span>Tab과 화살표 키로 quartile, whisker, fence와 outlier count를 확인할 수 있습니다.</span> : <><strong>{active.label}</strong>{details.map((detail) => <span key={detail.label}>{detail.label}: {detail.value}</span>)}</>}</div>
    </div>
  );
}
